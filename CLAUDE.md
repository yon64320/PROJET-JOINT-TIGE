...
| Migrations DB (squash)       | Done      | 001_schema.sql + 002_security_fixes.sql + seed.sql. 13 tables, 4 RPC (`merge_extra_column`, `delete_project_cascade`, `reimport_archive_lut`, `reimport_archive_jt`) avec checks `auth.uid()` internes, RLS stricte par owner, TEXT brut pour données Excel. Pipeline Supabase CLI (`db push`). |
