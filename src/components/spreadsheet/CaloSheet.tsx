...
const UniverSheet = dynamic(() => import("./UniverSheet"), {
  ...
  loading: () => (
    <div className="flex items-center justify-center h-full text-gray-500">
      Chargement du tableur Calorifuge...
    </div>
  ),
});
...
const EDITABLE_COL_INDICES = new Set(
  CALO_COLUMNS.map((c, i) => (c.editable ? i : -1)).filter((i) => i >= 0),
);
...
interface CaloSheetProps {
  rows: DbCaloFlange[];
}

const CALO_BASE_STYLES: Record<string, Record<string, unknown>> = {
  header: {
...
function buildWorkbookData(rows: DbCaloFlange[]): IWorkbookData {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const cellData: Record<number, Record<number, any>> = {};

  // En-têtes
  cellData[0] = {};
  CALO_COLUMNS.forEach((col, i) => {
  ...
  for (const [key, val] of Object.entries(CALO_BASE_STYLES)) {
  ...
  return {
    ...
    locale: 1,
    styles,
    sheetOrder: ["calo-sheet"],
    sheets: {
      "calo-sheet": {
        id: "calo-sheet",
        name: "Calorifuge",
        rowCount: rows.length + 2,
        columnCount: CALO_COLUMNS.length,
        defaultRowHeight: 24,
...
export default function CaloSheet({ rows }: CaloSheetProps) {
