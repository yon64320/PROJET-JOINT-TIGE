...
## 10. Les outils de ton quotidien

- **`GAMMES COMPILEES REV D.xlsm`** — le fichier maître. C'est la Bible de l'arrêt. 278 OTs, 2 794 phases, 242 équipements. Format xlsm (avec macros) mais exploité en `data_only` pour extraire les données.
...
*Document généré à partir de l'analyse des fichiers `GAMMES COMPILEES REV D.xlsm` et `Liste de levage - Copie.xlsx`, et des échanges de travail sur la préparation de l'arrêt.*
